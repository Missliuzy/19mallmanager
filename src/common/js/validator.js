...
function _auto_validate_ffzf(_input, _msg, _type, _this_config) {
  var _input_value = _input.val();
  // ie8下，select类型的输入框，val的返回值可能为数组
  var _input_value =
    typeof _input_value === 'object' ? _input_value[0] : _input_value;
  if (_input_value != '') {
...
var _validate_type_contact = /^((1\d{10})(\,|\，|\、)|(((\(\d{3,4}\))|(\d{3,4}\-))?[1-9]\d{6,7}(\(\d{1,3}\)|-\d{1,3})?)(\,|\，|\、))*((1\d{10})|(((\(\d{3,4}\))|(\d{3,4}\-))?[1-9]\d{6,7}(\(\d{1,3}\)|-\d{1,3})?))$/; // 多个手机号和分机号，用,，、分割
